        ...
        {topics.map(t => (
          <Link key={t.id} to={`/exam/guide/${t.id}`} className="card hover:border-blue-400 transition-colors block">
            <div className="flex items-center justify-between">
              ...
              {t.status.includes('jedro') && <span className="badge bg-red-100 text-red-700">jedro</span>}
            </div>
            <div className="text-xs text-stone-600 mt-1">{t.titleEn}</div>
            <div className="text-xs mt-2 text-stone-700 line-clamp-2">{t.legal.slice(0, 120)}…</div>
          </Link>
        ))}
      </div>
    </div>
...
export function TopicDetail() {
  ...
  return (
    <article className="space-y-4">
      <div>
        <Link to="/exam/guide" className="text-sm text-blue-600">← Vse teme</Link>
        <h1 className="text-xl font-bold mt-2">{t.n}. {t.title}</h1>
        <p className="text-sm text-stone-600">{t.titleEn} · status: {t.status}</p>
      </div>
      ...
      <section className="card border-blue-300">
      ...
      <section className="card">
        ...
        <ul className="text-sm list-disc pl-5 space-y-1">
          {t.materials.map(m => <li key={m} className="text-stone-700">{m}</li>)}
        </ul>
      </section>
          ...
          <ul className="text-sm space-y-2">
            {related.slice(0, 8).map(c => (
              <li key={c.id} className="border-l-2 border-stone-300 pl-3">
                ...
                <div className="text-stone-600 text-xs mt-0.5">{c.answer}</div>
