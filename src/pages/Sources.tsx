        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {sources.map(s => (
            <a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer" className="card hover:border-blue-400 block">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{s.body}</div>
            </a>
          ))}
        </div>
      </section>
          ...
          {docsManifest.map(d => (
            <div key={d.id} className="card flex items-center justify-between gap-3 !py-3">
              <div>
                ...
                <div className="text-xs text-slate-500">avtoriteta {d.authority} · {d.topics.join(', ') || 'splošno'}</div>
              </div>
